```
┌─────────────────────────────────────────────────────────────┐
│  HRC-1.02.08                                                  │
│  Puesto: Preparador / Empacador                               │
│  MARACUYA · Relaciones Humanas · Grado 1                      │
└─────────────────────────────────────────────────────────────┘

【 META - META 】
SOP No.    : HRC-1.02.08
Título     : Puesto: Preparador, Preprocesamiento y Empaque
Versión    : 1.0   Creado: 2026-07-06   Próxima revisión: 2027-01-06
Dueño (DRI): Operations Manager
Confianza  : T2
Banderas   : ⛑ SEGURIDAD
```

📌 **¿SABÍAS QUE?**
El preparador es el primer eslabón de orden en la cocina. Lo que llega crudo, lo limpia, lo porciona y lo empaca. Sin un buen preparador, el cocinero se atrasa y la comida se desperdicia.

`EN-[ The prep cook is the first link of order in the kitchen. What arrives raw, it cleans, portions and packs. Without a good prep cook, the line falls behind and food is wasted. ]`

### 【 PURPOSE - OBJETIVO 】
Definir al puesto que preprocesa, empaca y porciona insumos y excedentes.
`EN-[ Define the role that pre-processes, packs and portions inputs and surplus. ]`

### 【 SCOPE - ALCANCE 】
Incluye: limpieza de producto, porcionado, empaque, etiquetado, congelado de excedentes del personal.
Excluye: cocción y servicio (esos son del cocinero y el Chef).
Frecuencia: **DIARIO**.

### 【 PROCEDURE - PROCEDIMIENTO 】
**Responsabilidades principales**
1. Lavar y preparar producto según estándar ↳ see [[HSK-3.07.01]].
2. Porcionar y empacar para servicio y almacenamiento.
3. **ETIQUETAR** todo con fecha. Congelar y porcionar excedentes de comida del personal.
4. Mantener orden y rotación primero en entrar primero en salir.
5. Separar orgánicos para compostaje.

**Puestos que puede asumir**
- Apoyo de cocina, empacador, encargado de almacén frío, apoyo de inventario con Logistics Manager.

### 【 CONSEQUENCE - CONSECUENCIA 】
1. Falta de etiqueta y rotación causa **PRODUCTO ECHADO A PERDER**.
2. Empaque sucio arriesga la **INOCUIDAD**.

### 【 ESCALATION - ESCALAMIENTO 】
Reporta a: Chef.

### 【 BIBLE CHECK - REVISIÓN BIBLIA 】
✅ APROBADO. §4 separación, reduce desperdicio hacia el ciclo de residuos.

### 【 JERARQUÍA - HIERARCHY 】

Capa: **LAYER 2 — EQUIPOS DE DEPARTAMENTO**
Confianza: **T2 Standard**

**↰ REPORTA A (upstream):**
- ↰ [[HCY-1.02.06 - Chef]]

**↳ DIRIGE (downstream):**
- (sin subordinados directos)

**≈ DEPARTAMENTOS CONECTADOS:**
- [[DEPT-FNB]] · [[DEPT-WST]] · [[DEPT-LOG]]

**📋 Mapa completo:** ↳ see [[HCY-1.01.01 - HEIRARCHY]]
```
```yaml
sop_id: HRC-1.02.08
title_es: "Puesto: Preparador, Preprocesamiento y Empaque"
dept: HRC
grade: 1
unit: "HRC-1.02"
tier: "T2"
type: "STANDARD"
roles_owner: "Operations Manager"
bible_check: pass
critical_system: false
operator_scope: false
interlock_pending: true
```