
```
┌─────────────────────────────────────────────────────────────┐
│  HRC-1.02.05                                                  │
│  Puestos de Apoyo Legal y Financiero                          │
│  MARACUYA · Relaciones Humanas · Grado 1                      │
└─────────────────────────────────────────────────────────────┘

【 META - META 】
SOP No.    : HRC-1.02.05
Título     : Puestos: Abogados, Despacho Contable y Compañía Operadora
Versión    : 1.0   Creado: 2026-07-06   Próxima revisión: 2027-01-06
Dueño (DRI): Operations Manager
Confianza  : T0
Banderas   : 💵 EFECTIVO · 🔒 PII
```

📌 **¿SABÍAS QUE?**
Estos puestos son externos y de **ALCANCE COMPAÑÍA OPERADORA**. El staff de campo casi nunca los ve, pero protegen a toda la finca desde el marco legal y financiero.

`EN-[ These roles are external and OPERATOR COMPANY SCOPE. Field staff rarely see them, but they protect the whole property from the legal and financial frame. ]`

### 【 PURPOSE - OBJETIVO 】
Definir los tres puestos de soporte externo y qué cubre cada uno.
`EN-[ Define the three external support roles and what each covers. ]`

### 【 SCOPE - ALCANCE 】
Incluye: legal laboral, penal y administrativo, contabilidad, y la operación de cumplimiento entrante.
Excluye: operación diaria de la finca.
Frecuencia: **CONTINUA Y POR EVENTO**.

### 【 PROCEDURE - PROCEDIMIENTO 】
**Abogados x3** ([[ROLE-Lawyers]])
- **Laboral** ([[ROLE-Lawyer-Labor]]): contratos, IMSS, relaciones de personal.
- **Penal** ([[ROLE-Lawyer-Penal]]): seguridad, incidentes graves, responsabilidad.
- **Administrativo** ([[ROLE-Lawyer-Administrative]]): permisos, propiedad, gobierno.

**Despacho Contable** ([[ROLE-Accounting-Firm]])
- Contabilidad, facturación, nómina bajo cumplimiento.

**Compañía Operadora** ([[ROLE-Operator-Company]]) entrante
- Absorbe finanzas, nómina, cumplimiento, seguros y contratos.

### 【 CONSEQUENCE - CONSECUENCIA 】
1. Sin soporte legal, un incidente puede volverse **RIESGO LEGAL MAYOR** ⚠️.
2. Sin cumplimiento, la operación no puede tener seguro ni contratos.

### 【 ESCALATION - ESCALAMIENTO 】
Todos reportan a: [[ROLE-Executor-Administrator]]. [ALCANCE COMPAÑÍA OPERADORA].

### 【 BIBLE CHECK - REVISIÓN BIBLIA 】
✅ APROBADO. Soporte administrativo, sin impacto ecológico directo.
```
```yaml
sop_id: HRC-1.02.05
title_es: "Puestos: Abogados, Despacho Contable y Compañía Operadora"
dept: HRC
grade: 1
unit: "HRC-1.02"
tier: "T0"
type: "STANDARD"
roles_owner: "Operations Manager"
bible_check: pass
critical_system: false
operator_scope: true
interlock_pending: true
```

---
